...
export class DialogNoteToColumnComponent implements OnInit {
  ...
  onSaveClick(): void {
    ...
    this.hostServ.toNoteColumn(this.noteId, this.data.columnId).subscribe({
      next: res => {
        if (res.success) {
          this.notifyServ.notify(`笔记[${this.data.name}]收藏成功！！！`, 'success');
          this.dialogRef.close({ op: 'save', data: this.data });
        } else {
          const msg = `笔记[${this.data.name}]收藏失败！！！ ${res.allMessages}`;
          this.notifyServ.notify(msg, 'error');
        }
      },
      error: err => {
        const msg = `笔记[${this.data.name}]收藏失败！！！ ${err}`;
        this.notifyServ.notify(msg, 'error');
      }
    });
  }
    ...
    this.hostServ.getColumnItems().subscribe({
      next: res => {
        if (res.success) {
          ...
          const msg = `收藏专栏选项加载失败！！！ ${res.allMessages}`;
          this.notifyServ.notify(msg, 'error');
        }
      },
      error: err => {
        const msg = `收藏专栏选项加载失败！！！ ${err}`;
        this.notifyServ.notify(msg, 'error');
      }
    });
  }
