...
export class DialogDomainComponent implements OnInit {
  ...
  private onAdd(): void {
    this.hostServ.addDomain(this.data).subscribe({
      next: res => {
        if (res.success) {
          this.notifyServ.notify(`添加领域[${this.data.name}]成功！！！`, 'success');
          this.dialogRef.close({ op: 'save', data: res.data });
        } else {
          const msg = `添加领域[${this.data.name}]失败！！！ ${res.allMessages}`;
          this.notifyServ.notify(msg, 'error');
        }
      },
      error: err => {
        const msg = `添加领域[${this.data.name}]失败！！！ ${err}`;
        this.notifyServ.notify(msg, 'error');
      }
    });
  }
    ...
    this.hostServ.updateDomain(this.data).subscribe({
      next: res => {
        if (res.success) {
          ...
          const msg = `更新领域[${this.origionName}]失败！！！ ${res.allMessages}`;
          this.notifyServ.notify(msg, 'error');
        }
      },
      error: err => {
        const msg = `更新领域[${this.origionName}]失败！！！ ${err}`;
        this.notifyServ.notify(msg, 'error');
      }
    });
  }
